...
router.post('/send_message', async (req, res) => {
    ...
    try{
        ...
        setTimeout(handleNotification, 1000, false, receiver_id, '', process.env.APP_NAME, subTitle, notification, "message");
...
router.post('/get_conversations', async (req, res) => {
    ...
    try{
        ...
        for(let i = 0; i < conversations.length; i++){
            ...
            const otherUser = await DeliveryAgent.findOne({_id: other_user_id}).lean();

            conversations[i].other_username = otherUser.fullname;
            conversations[i].other_user_img = otherUser.img_url;
        }
